 ...
 */
package me.almzmlimam.mahasiswa.model;
...
public class MahasiswaDAO {
    ...
    public MahasiswaDAO() {
        try {
            ...
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/mvc_50421097", "root", "");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void updateMahasiswa(ModelMahasiswa mahasiswa) {
        String sql = "UPDATE mahasiswa SET npm = ?, nama = ?, semester = ?, ipk = ? WHERE id = ?"; // Koma berlebih dihapus
        try {
            PreparedStatement pstmt = connection.prepareStatement(sql);
            pstmt.setString(1, mahasiswa.getNpm());
            pstmt.setString(2, mahasiswa.getNama());
            pstmt.setInt(3, mahasiswa.getSemester());
            pstmt.setFloat(4, mahasiswa.getIpk());
            pstmt.setInt(5, mahasiswa.getId()); // Parameter kelima dihapus
            pstmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
